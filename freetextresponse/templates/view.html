...
<div class="freetextresponse xmodule_display xmodule_CapaModule problem">
    ...
    <div class="capa_inputtype textline">
      {% if not is_graded %}
        ...
        {%else%}
          <p>Your answer is {{student_answer}}</p>
          <p>Grade scored is {{ problem_progress }}</p>
          <p>Instructor comments are {{student_submission.comment}} </p>
        {%endif%}
    </div>
...
<section aria-hidden="true" class="modal staff-modal grade-submission" id="grade-{{block_id}}" style="height: auto; position: fixed; opacity: 1; z-index: 11000; left: 30%; margin-left: -402px; top: 22px; max-height: 600px;" tabindex="-1">
    <div class="inner-wrapper" style="color: black; overflow: auto;">
      ...
      <br>
      <div>
        <a class="instructor-info-action button btn-download-all"  onclick="exportTableToExcel('{{block_id}}');" id="download-init-button">Download All Submissions</a>
      </div>
      ...
      <tbody>
            {% for submission in users_submissions.submissions %}
                <tr>
                    ...
                    <td><span style="display: block; width: 100px; overflow: hidden; white-space: nowrap; text-overflow: ellipsis;border: none;">{{submission.student_answer}}</span></td>
                    {% if submission.score %}
